...
class BinocularSavingVideo(Node):
    def __init__(self):
        super().__init__("binocular_saving_video")
        self.bridge = CvBridge()

        self.subscription = self.create_subscription(Image, 'cam/binocular', self.display_n_save, 1)

        self.video_path = os.path.join(saving_path, f"binocular_cam_{now_moment}.avi")
...
def main(args=None):
    ...
    binocular_video = BinocularSavingVideo()
    try:
        rclpy.spin(binocular_video)
    except:
        binocular_video.destroy_node()
        rclpy.shutdown()
