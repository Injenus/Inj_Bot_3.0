...
Когда подъедм на близкое растсояние следует повернуть, но будем двигаться БЕЗ повротов - омни колёса - 
учитвая, что лидар способен смоеть на все четыре стороны с центром и боковыми.
Таким образом двигаемся по всем повортоам.

Когда дотсигаем лекарств (хуй знает че делать), исходя из постоянства лабиринта, поворачиемся до тех пор, 
пока в нужна арука не будет в центре кадра.
Если арука в центре кадра, но передние два ближайших к центры разные -  занчит стоим криво. 
Поворачиемваемся до тех пор, пока расстония не будут примерно равны,
...
class PharmaDelivery(Node):
    def __init__(self):
        ...
        self.side_idx_by_dist = {10000: 1, 1000: 2, 500: 3, 100: 4}
    ...
    def get_dist_idx(self, dist):
        sorted_keys = sorted(self.side_idx_by_dist.keys(), reverse=True)

        index = bisect.bisect_left(sorted_keys, dist, lo=0, hi=len(sorted_keys))
        if index < len(sorted_keys):
            return self.side_idx_by_dist[sorted_keys[index]]
        else:
            return self.side_idx_by_dist[sorted_keys[-1]]
    ...
    def loop(self):
        ...
        if self.main_state == 2:
            ...
            if False:
                ...
                idx = self.get_dist_idx(self.lidar_basic[key][1])
                try:
                    if key in ['front', 'back']:
                        if idx == 1 or True:
                            # self.error = self.lidar_basic[key][0] - self.lidar_basic[key][2]
                            self.error = self.lidar_all[conf.LIDAR_STEP*3] - self.lidar_all[conf.LIDAR_STEP*4]
                        else:
                            pass
                            # if key == 'front':
                            #     self.error = self.lidar_all[360-idx*conf.LIDAR_STEP] - self.lidar_all[idx*conf.LIDAR_STEP]                        
                            # else:
                            #     self.error = self.lidar_all[180-idx*conf.LIDAR_STEP] - self.lidar_all[180+idx*conf.LIDAR_STEP]
                        # if self.error < 0:
                        #     self.error = 0.001*self.error
                    if key == 'left':
                        if idx == 1:
                            self.error = self.lidar_basic['left'][1] * 1.00375 - self.lidar_basic['left'][2] # 1.00375 СТРОГО ДЛЯ 10град.!!!!!!!!
                        else:
                            self.error = self.lidar_all[270-idx*conf.LIDAR_STEP] - self.lidar_all[270+idx*conf.LIDAR_STEP]
                    if key in ['right', 'right_one_cell']:
                        if idx == 1:
                            ...
                            self.error = self.lidar_all[90-idx*conf.LIDAR_STEP] - self.lidar_all[90+idx*conf.LIDAR_STEP]
                ...
                if self.turn_counter in [0, 2]:
                    twist_msg.angular.z = self.constrain(angular_z, -0.2, 1.9)
                else:
                    twist_msg.angular.z = self.constrain(angular_z, -0.2, 0.2)                    
